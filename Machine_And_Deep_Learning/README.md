<h1>Computer Science → AI → ML → DL</h1>

<table style="width:100%; border-collapse: collapse; text-align: left;">

<tr>
    <th style="width:15%; padding:10px;">Full Name</th>
    <th style="width:22%; padding:10px;">What it does</th>
    <th style="width:25%; padding:10px;">How it works</th>
    <th style="width:18%; padding:10px;">Specific Example</th>
    <th style="width:12%; padding:10px;">Tools / Software</th>
    <th style="width:8%; padding:10px;">Relationship</th>
</tr>

<tr>
    <td style="padding:10px;"><b>Computer Science (CS)</b></td>
    <td style="padding:10px;">
        Create programs/software to solve problems using human-written instructions
    </td>
    <td style="padding:10px;">
        1. Programmer writes code<br>
        2. Computer reads code<br>
        3. Executes instructions<br>
        4. Produces output
    </td>
    <td style="padding:10px;">
        Calculator app: 5 + 3 → output 8
    </td>
    <td style="padding:10px;">
        Python, Java, C++, Git, VS Code
    </td>
    <td style="padding:10px;">
        Largest field
    </td>
</tr>

<tr>
    <td style="padding:10px;"><b>Artificial Intelligence (AI)</b></td>
    <td style="padding:10px;">
        Make computers perform tasks that normally require human decision-making
    </td>
    <td style="padding:10px;">
        1. Input enters system<br>
        2. AI checks rules/logic<br>
        3. Chooses action<br>
        4. Returns output
    </td>
    <td style="padding:10px;">
        Chess AI selects best move
    </td>
    <td style="padding:10px;">
        Python, Prolog, OpenCV
    </td>
    <td style="padding:10px;">
        Subset of CS
    </td>
</tr>

<tr>
    <td style="padding:10px;"><b>Machine Learning (ML)</b></td>
    <td style="padding:10px;">
        Make computers learn from data to predict future results
    </td>
    <td style="padding:10px;">
        1. Collect data<br>
        2. Train model<br>
        3. Learn from examples<br>
        4. Predict new data
    </td>
    <td style="padding:10px;">
        Email spam detection
    </td>
    <td style="padding:10px;">
        Python, Scikit-learn, Pandas
    </td>
    <td style="padding:10px;">
        Subset of AI
    </td>
</tr>

<tr>
    <td style="padding:10px;"><b>Deep Learning (DL)</b></td>
    <td style="padding:10px;">
        Use neural networks to learn complex data automatically
    </td>
    <td style="padding:10px;">
        1. Prepare large dataset<br>
        2. Build neural network<br>
        3. Train model<br>
        4. Predict output
    </td>
    <td style="padding:10px;">
        Cat image recognition
    </td>
    <td style="padding:10px;">
        PyTorch, TensorFlow, CUDA
    </td>
    <td style="padding:10px;">
        Subset of ML
    </td>
</tr>

</table>

<h2>Hierarchy</h2>

<pre>
Computer Science
└── Artificial Intelligence
    └── Machine Learning
        └── Deep Learning
</pre>
